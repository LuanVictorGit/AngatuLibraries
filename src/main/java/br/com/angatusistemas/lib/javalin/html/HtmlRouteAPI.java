 ...
 * <p>
 * Escaneia a pasta /public (classpath ou sistema de arquivos, conforme configuração do Javalin)
 * em busca de arquivos .html e registra rotas dinâmicas baseadas no nome do arquivo.
 * Suporta template com placeholders, cache de conteúdo, exclusão de páginas e personalização.
 * </p>
 *
 * [EN] Utility for automatic registration of HTML routes from static files.
 * <p>
 * Scans the /public folder (classpath or filesystem, depending on Javalin config)
 * for .html files and registers dynamic routes based on file names.
 * Supports template with placeholders, content caching, page exclusion and customization.
 * </p>
 */
public final class HtmlRouteAPI {
    ...
    /**
     * [PT] Ativa/desativa o cache das páginas HTML (ativo por padrão).
     * [EN] Enables/disables HTML page cache (enabled by default).
     */
    public static void setCacheEnabled(boolean enabled) {
        cacheEnabled = enabled;
        if (!enabled) PAGE_CACHE.clear();
        Console.debug("Cache de rotas HTML " + (enabled ? "ativado" : "desativado"));
    }

    /**
     * [PT] Define o tempo de vida do cache (TTL) em milissegundos.
     * [EN] Sets the cache time-to-live (TTL) in milliseconds.
     */
    ...
    /**
     * [PT] Remove um path da lista de ignorados.
     * [EN] Removes a path from the ignored list.
     */
    ...
    /**
     * [PT] Define a lista de paths ignorados (substitui a atual).
     * [EN] Sets the list of ignored paths (replaces current).
     */
    ...
    /**
     * [PT] Registra automaticamente todas as rotas HTML a partir dos arquivos estáticos.
     * <p>
     * Para cada arquivo .html na pasta /public (exceto os ignorados e o template base,
     * se houver), cria uma rota GET no formato "/nome_do_arquivo".
     * Utiliza um template base (ex: index.html) para envolver o conteúdo da página.
     * Os placeholders suportados são:
     * <ul>
     ...
     * <p>
     * For each .html file in the /public folder (except ignored ones and the base template),
     * creates a GET route in the format "/file_name".
     * Uses a base template (e.g., index.html) to wrap the page content.
     ...
     * </p>
     *
     * @param javalin         [PT] Instância do Javalin onde as rotas serão registradas
     *                        [EN] Javalin instance where routes will be registered
     * @param baseTemplatePath [PT] Caminho do template base (ex: "/index.html") – pode ser null
     *                        [EN] Path to the base template (e.g., "/index.html") – may be null
     * @param pageProvider    [PT] Provedor de lista de páginas (opcional, se null usa {@link #getAllHtmlPages()})
     *                        [EN] Page list provider (optional, if null uses {@link #getAllHtmlPages()})
     * @param contentLoader   [PT] Carregador de conteúdo personalizado (opcional)
     *                        [EN] Custom content loader (optional)
     */
    public static void registerAllRoutes(Javalin javalin, String baseTemplatePath,
                                          PageProvider pageProvider, ContentLoader contentLoader) {
        ...
        for (String filePath : htmlFiles) {
            ...
            javalin.unsafe.routes.get(routePath, ctx -> {
                try {
                    ...
                    String renderedHtml;
                    if (finalBaseHtml != null) {
                        renderedHtml = finalBaseHtml
                                .replace("{page}", StringAPI.capitalize(pageName))
                                .replace("{content}", pageContent);
                        // Substitui placeholders de menu ativo
    ...
    public static List<String> getAllHtmlPages() {
        return AssetsAPI.listAllAssetsRecursive("/").stream()
                .filter(file -> file.endsWith(".html"))
                .filter(file -> {
                    String normalized = file.replace("\\", "/");
                    return !normalized.contains("/emails/")
                            && !normalized.contains("/others/");
                })
                .collect(Collectors.toList());
    }

    /**
     * [PT] Extrai o nome da página a partir do caminho do arquivo (ex: "/sobre.html" -> "sobre").
     * [EN] Extracts the page name from the file path (e.g., "/about.html" -> "about").
     */
